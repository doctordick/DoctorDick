...
import { PUSH_NEW_ROUTE, POP_ROUTE, POP_TO_ROUTE, REPLACE_ROUTE, REPLACE_OR_PUSH_ROUTE, PUSH_NEW_ROUTE_PARAMS } from '../actions/route';
...
export default function (state:State = initialState, action:Action): State {
  ...
  // console.log(state, "route state *()*(*&77");
  if (action.type === PUSH_NEW_ROUTE_PARAMS) {
    action.params.id = action.route;
    globalNav.navigator.push(action.params);
    return {
      routes: [...state.routes, action.route]
    };
  }
